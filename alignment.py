...
def main():
    ...
    im = cv2.imread("image/emir.jpg", cv2.IMREAD_GRAYSCALE)
    # Find the width and height of the color image
    sz = im.shape
    print("image shape ", sz)
    height = int(sz[0] / 3)
    width = sz[1]
    ...
    cv2.waitKey(0)
    cv2.imwrite("image/emir_unaligned.png", im_color)
    cv2.imwrite("image/emir_aligned.png", im_aligned)
    return
